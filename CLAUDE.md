...
5. **Zod validation** — only at API route boundary. Domain validation via `validateFn` in `src/domain/`.
6. **Unit tests** — always mock repositories with `vi.fn()`. No real DB connections. Test folder: `__test__/application/usaCases/` (legacy typo — keep it).
